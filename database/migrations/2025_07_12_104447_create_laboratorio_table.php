...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('laboratorio', function (Blueprint $table) {
            ...
            $table->string('antcod_peec', 15)->nullable();
            $table->string('numsedes_lab', 15)->nullable();
            $table->bigInteger('nit_lab')->nullable();
            $table->string('nombre_lab', 254);
            $table->string('sigla_lab', 20)->nullable();
            $table->unsignedBigInteger('id_nivel');
            $table->unsignedBigInteger('id_tipo');
            $table->unsignedBigInteger('id_categoria');
            $table->string('respo_lab', 100);
            $table->string('ci_respo_lab', 25)->nullable();
            $table->string('repreleg_lab', 100);
            $table->string('ci_repreleg_lab', 25)->nullable();
            $table->unsignedBigInteger('id_pais');
            $table->unsignedBigInteger('id_dep');
            $table->unsignedBigInteger('id_prov');
            $table->unsignedBigInteger('id_municipio');
            $table->string('zona_lab', length: 100);
            $table->string('direccion_lab', 254);
            $table->string('wapp_lab', 50);
            $table->string('wapp2_lab',50)->nullable();
            $table->string('mail_lab', 80);
            $table->string('mail2_lab', 80)->nullable();
