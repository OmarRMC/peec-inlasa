...
class PaqueteController extends Controller
{
    ...
    public function getPaquetesPorPrograma(Request $request)
    {
        ...
        foreach ($paquetesDB as $paquete) {
            $paquetes->push([
                'id' => $paquete->id,
                'nombre_paquete' =>  $paquete->descuento > 0  ? "{$paquete->descripcion} {$paquete->descuento_html}" : $paquete->descripcion,
                'costo' => $paquete->precio_final,
                'descuento' => $paquete->descuento,
                'costo_con_descuento' => $paquete->descuento > 0 ? "{$paquete->costo_paquete} Bs <br/> {$paquete->precio_final_html}"   : $paquete->costo_paquete,
